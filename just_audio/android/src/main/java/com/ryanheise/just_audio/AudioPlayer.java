...
public class AudioPlayer implements MethodCallHandler, Player.Listener, MetadataOutput {
    ...
    public void onTracksChanged(Tracks tracks) {
        for (int i = 0; i < tracks.getGroups().size(); i++) {
            ...
            for (int j = 0; i < trackGroup.length; i++) {
    ...
    private void ensurePlayerInitialized() {
        if (player == null) {
            RenderersFactory audioRenderer = (
                    handler,
                    ...
                    new MediaCodecAudioRenderer(context, MediaCodecSelector.DEFAULT, handler, audioListener, AudioCapabilities.DEFAULT_AUDIO_CAPABILITIES),
                    new LibflacAudioRenderer(handler, audioListener)
            };

            ExoPlayer.Builder builder = new ExoPlayer.Builder(context, audioRenderer);
            builder.setMediaSourceFactory(new DefaultMediaSourceFactory(context, extractorsFactory));
            builder.setWakeMode(C.WAKE_MODE_LOCAL);
            if (loadControl != null) {
